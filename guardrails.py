#!/usr/bin/env python3
"""
IAM Sentinel — CI/CD guardrails scanner (offline, no server required).

The PR-gate: scan IAM policies (or the IAM inside a Terraform plan) and FAIL the build when a
privilege-escalation / over-permission finding at or above a threshold is present.

Examples:
  # Scan IAM inside a Terraform plan (run in CI after `terraform plan -out tf.plan`
  #                                   then `terraform show -json tf.plan > plan.json`)
  python3 guardrails.py --tfplan plan.json --fail-on HIGH

  # Scan raw policy JSON files / a directory
  python3 guardrails.py policies/*.json --fail-on CRITICAL

  # Emit SARIF for GitHub code scanning (inline PR annotations)
...
def main():
    ...
    ap.add_argument("--format", default="text", choices=["text", "json", "sarif"])
    args = ap.parse_args()
    ...
    if not items:
        ...
        print(json.dumps(_sarif([])) if args.format == "sarif" else "")
        sys.exit(0)
